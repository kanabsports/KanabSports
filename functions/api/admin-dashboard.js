...
export async function onRequestGet({request,env}){
  ...
  const verified=await env.SPORTS_DB.prepare(`SELECT COUNT(*) AS count FROM coach_access_requests WHERE status='approved'`).first(),last=await env.SPORTS_DB.prepare(`SELECT MAX(COALESCE(published_at,reviewed_at,created_at)) AS at FROM coach_submissions WHERE status='approved'`).first(),adminUser=await env.SPORTS_DB.prepare(`SELECT email FROM admin_users WHERE email=? LIMIT 1`).bind(OWNER_EMAIL).first();
  return json({authenticated:true,user:{email:session.email,role:'Owner',hasPassword:Boolean(adminUser)},stats:{pending:items.length,verifiedCoaches:Number(verified?.count||0),updatesNeeded:0,lastPublished:last?.at||null},items,recent:recent.results||[],health:{database:true,email:Boolean(env.RESEND_API_KEY),site:true}});
}
